...
import com.codeflyz.documents.ElementProcessorWorkBook;
 ...
 * @author wilfongj
 */
public class Assemble_Merge_WorkBook_001 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(prj.getProjectPath());
        System.out.println(prj.getOutputFileName());
        ...
        // write the start of the excel file
        ...
        for (int i = 0; i < me.size(); i++) {
            ...
            prj.setOutputFileName(project_path);
            ElementProcessorWorkBook EP = new ElementProcessorWorkBook(prj, ES);
            prj.setOutputFileName(prj.getProjectName() + "." + ES.getElement(0).getId() + "." + ES.getProjectName() + ".xml");
            File wbf = new File(prj.getOutputPath(), prj.getOutputFileName());
            if (wbf.exists()) {
                wbf.delete();
            }
            System.out.println("workbook: " + prj.getOutputFileName());
            ProcessLogger pwb = new ProcessLogger(prj.getOutputPath(), prj.getOutputFileName());
            pwb.WriteNoDate(wb.formatWorkBookStart());
            
            EP.process();
            
            pwb.WriteNoDate(wb.formatWorkBookEnd());
        }//for
